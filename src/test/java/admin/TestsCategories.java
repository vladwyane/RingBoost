package admin;

import admin.categories.Categories;
import admin.role.Roles;
import admin.testData.CategoriesData;
import admin.testData.RolesData;
import base.TestBase;
import com.fasterxml.jackson.databind.ObjectMapper;
import io.restassured.RestAssured;
import io.restassured.response.Response;
import io.restassured.specification.RequestSpecification;
import org.json.JSONObject;
import org.testng.Assert;
import org.testng.annotations.BeforeClass;
import org.testng.annotations.Test;

import java.io.IOException;

/**
 * Created by bigdrop on 2/26/2019.
 */
public class TestsCategories extends TestBase {

    RequestSpecification request = RestAssured.given();
    ObjectMapper mapper = new ObjectMapper();
    Categories categories = new Categories();
    int idLastCategory;
    int amountCategoryBefore;
    int amountCategoryAfter;

    @BeforeClass()
    public void requestListOfCategories() throws IOException {
        request = getToken();
        Response response = request.get(RequestURI.CATEGORIES_URI);
        JSONObject jsonObject = new JSONObject(response.asString());
        categories = mapper.readValue(jsonObject.toString(), Categories.class);
        idLastCategory = categories.getModels().get(categories.getModels().size() - 1).getId();
        amountCategoryBefore = categories.getModels().size();
    }

    @Test()
    public void getListOfCategories() throws IOException {
        Assert.assertEquals(categories.getResult(), "success");
    }


    @Test()
    public void successCreateCategory() throws IOException {
        request.body(categories.addParamToBodyForCreateCategory(CategoriesData.CHRISTMAS));
        Response response = request.post(RequestURI.CATEGORIES_URI);
        Assert.assertEquals(response.statusCode(), 201);
    }

    @Test()
    public void errorCreateCategoryWithExistName() throws IOException {
        request.body(categories.addParamToBodyForCreateCategory(CategoriesData.CHRISTMAS));
        Response response = request.post(RequestURI.CATEGORIES_URI);
        Assert.assertEquals(response.statusCode(), 422);
    }

    @Test()
    public void getNameLastCategory() throws IOException {
        Response response = request.get(RequestURI.CATEGORIES_URI);
        JSONObject jsonObject = new JSONObject(response.asString());
        categories = mapper.readValue(jsonObject.toString(), Categories.class);
        String nameLastRole = categories.getModels().get(categories.getModels().size() - 1).getName();
        Assert.assertEquals(nameLastRole, CategoriesData.CHRISTMAS.getName());
    }

    @Test()
    public void getListOfCategoriesAfterCreateNewCategory() throws IOException {
        amountCategoryAfter = categories.getModels().size();
        idLastCategory = categories.getModels().get(amountCategoryAfter - 1).getId();
        Assert.assertEquals(amountCategoryBefore + 1, amountCategoryAfter);
    }

    @Test()
    public void getLastCategoryById() throws IOException {
        Response response = request.get(RequestURI.CATEGORIES_URI + idLastCategory);
        Assert.assertEquals(response.statusCode(), 200);
    }

    @Test()
    public void successUpdateCategory() throws IOException {
        request.body(categories.addParamToBodyForCreateCategory(CategoriesData.CATEGORY_UPDATE));
        Response response = request.patch(RequestURI.CATEGORIES_URI + idLastCategory);
        Assert.assertEquals(response.statusCode(), 200);
    }

    @Test()
    public void getNameLastCategoryAfterUpdate() throws IOException {
        Response response = request.get(RequestURI.CATEGORIES_URI);
        JSONObject jsonObject = new JSONObject(response.asString());
        categories = mapper.readValue(jsonObject.toString(), Categories.class);
        String nameLastRole = categories.getModels().get(categories.getModels().size() - 1).getName();
        Assert.assertEquals(nameLastRole, CategoriesData.CATEGORY_UPDATE.getName());
    }

    @Test()
    public void successDeleteCategory() throws IOException {
        Response response = request.delete(RequestURI.CATEGORIES_URI + idLastCategory);
        Assert.assertEquals(response.statusCode(), 204);
    }

    @Test()
    public void getListOfCategoriesAfterDelete() throws IOException {
        Response response = request.get(RequestURI.CATEGORIES_URI);
        JSONObject jsonObject = new JSONObject(response.asString());
        categories = mapper.readValue(jsonObject.toString(), Categories.class);
        Assert.assertEquals(categories.getModels().size(), amountCategoryBefore);
    }
}
